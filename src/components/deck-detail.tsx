...
function AddWordForm({onSave,onCancel}:{onSave:(word:Omit<Word,"id">)=>void;onCancel:()=>void}){
 ...
 return <form className="panel add-word-form" onSubmit={submit}><h2>Thêm một từ, mở một cánh cửa.</h2><fieldset disabled={saving} className="add-word-fields"><div className="form-grid"><PronunciationFields value={value} onChange={setValue}/><label>Nghĩa tiếng Việt<input name="meaning" required maxLength={1000} value={meaning} onChange={e=>setMeaning(e.target.value)} placeholder="truyền cảm hứng"/></label><MeaningLookup term={value.term} onSelect={setMeaning}/><label>Câu ví dụ (không bắt buộc)<input name="example" maxLength={250} placeholder="Small steps inspire big changes."/></label></div><div className="row"><button className="button primary" disabled={saving}>{saving?"Đang lưu…":"Lưu từ mới"}</button><button className="button secondary" type="button" onClick={onCancel}>Hủy</button></div></fieldset></form>;
}
