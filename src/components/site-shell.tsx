...
export function SiteHeader() {
  const path = usePathname(); const { state } = useLearning();
  return <><a className="skip-link" href="#main">Đến nội dung chính</a><header className="site-header"><div className="header-inner"><Brand /><nav aria-label="Điều hướng chính">{links.map(link => <Link key={link.href} href={link.href} className={path === link.href || (link.href !== "/" && path.startsWith(link.href)) ? "active" : ""} aria-current={path === link.href ? "page" : undefined}><Icon name={link.icon} size={17} />{link.label}</Link>)}</nav><Link href={state.name ? "/dashboard" : "/login"} className="account-link"><span className="avatar">{state.name ? state.name.charAt(0).toUpperCase() : <Icon name="user" size={16} />}</span><span>{state.name || "Đăng nhập"}</span><Icon name="chevron" size={14} /></Link></div></header></>;
}
