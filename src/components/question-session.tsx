...
import { QUIZ_QUESTION_MS,begin,step,stateSchema,publicState,score,type Command } from "@/lib/session-engine";
...
export default function QuestionSession({id}:{id:string;quiz?:boolean}){
 ...
 function start(){
  ...
  if(!record)record={id:crypto.randomUUID(),title:`Quiz: ${deck.title}`,state:begin(makeQuiz(deck).slice(0,100),true,QUIZ_QUESTION_MS,Date.now())};
 ...
 if(account)return <main id="main" className="container page-main server-session"><section className="panel"><h1>{deck.title}</h1><p>10 giây mỗi câu. Kết quả lưu vào tài khoản sau khi hoàn thành.</p><StartButton kind="QUIZ" sourceId={id}/></section></main>;
 return <main id="main" className="container page-main server-session">{active?<SessionPlayer key={active.id} id={active.id} title={active.title} execute={execute} local/>:<section className="panel"><span className="badge">QUIZ CÁ NHÂN</span><h1>{deck.title}</h1><p>Mỗi câu có 10 giây. Chọn đáp án để xem đúng/sai; sau 1,10 giây sẽ tự chuyển câu. Thời gian vẫn tính khi rời tab.</p><p>Phiên và kết quả của bộ từ cá nhân này được lưu trên trình duyệt đang dùng.</p><button className="button primary" onClick={start}>Bắt đầu / tiếp tục quiz</button><p role="status">{error}</p></section>}</main>;
}
