...
export const contentInput = z.object({
 ...
 if(new TextEncoder().encode(JSON.stringify(v)).length>750000)ctx.addIssue({code:"custom",message:"Đề quá dài. Hãy chia thành các đề nhỏ hơn."});
 const groups=new Map<string,string>();
 for(const q of v.questions)if(q.reading){
  const serialized=JSON.stringify(q.reading);
  if(groups.has(q.reading.id)&&groups.get(q.reading.id)!==serialized)ctx.addIssue({code:"custom",path:["questions"],message:"Các câu cùng nhóm phải dùng cùng nội dung bài đọc"});
  groups.set(q.reading.id,serialized);
 }
 const numbers=v.questions.map((q,i)=>q.number??i+1);
 if(new Set(numbers).size!==numbers.length)ctx.addIssue({code:"custom",path:["questions"],message:"Số câu hỏi không được trùng nhau"});
