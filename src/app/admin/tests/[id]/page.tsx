...
export default async function EditorPage({params}:{params:Promise<{id:string}>}){
 ...
 const initial:ContentInput=item?{...item,level:contentInput.shape.level.parse(item.level),color:contentInput.shape.color.parse(item.color),version:item.updatedAt.toISOString(),kind:"test",published:item.published,words:[],questions:item.questions,minutes:item.minutes,symbol:"Aa"}:{kind:"test",title:"",description:"",category:"",level:"A1",color:"blue",symbol:"Aa",published:false,minutes:5,words:[],questions:[]};
